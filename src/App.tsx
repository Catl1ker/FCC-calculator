...
function App() {
  ...
  const buttonPress = (symbol: string) => {
    if (symbol === "clear") {
      ...
      if (symbol === "-" && et.charAt(et.length - 1) !== "-") {
        setExpression(et + " " + symbol + " ");
      } else if (isOperator(et.charAt(et.length - 1))) {
        if (isOperator(et.charAt(et.length - 3))) {
          const newEt = et.replace(/.{3}$/, symbol + " ");
          setExpression(newEt);
          return;
        } else {
          const newEt = et.replace(/.$/, symbol + " ");
          setExpression(newEt);
          return;
        }
  ...
  return (
    <div className="container">
      ...
      <div id="calculator">
        ...
        <button id="divide" onClick={() => buttonPress("/")} className="orange">
          /
        </button>
        <button
          id="seven"
        ...
        <button id="equals" onClick={() => buttonPress("=")} className="orange">
          =
        </button>
      </div>
    </div>
